...
const GamesPage = (props) => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/api/allGames')
      .then((res) => {
        console.log('Line 12: GamesPage.jsx ', res.data);
  ...
  const returnHandler = (rentalId) => {
    axios.put(`http://localhost:8000/api/return/${rentalId}`, {}, { withCredentials: true })
      .then((res) => {
            ...
            <tr key={game.id}>
              ...
              <td>
                {game.isRented && game.userId === currentUser.id ? (
                  <button onClick={() => returnHandler(game.id)} className='btn btn-warning'>Return</button>
                ) : (
