...
const GamesPage = (props) => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8000/api/allGames')
            .then((res) => {
                ...
                const allGenres = Array.from(new Set(res.data.flatMap(game => game.genre.split(', '))));
    ...
    useEffect(() => {
        if (selectedGenre) {
            ...
            setFilteredGames(games.filter(game => game.genre.includes(selectedGenre)));
        } else {
    ...
    const rentalHandler = (gameId) => {
        axios.post(`http://localhost:8000/api/rental/${gameId}`, { userId: currentUser.id }, { withCredentials: true })
            .then((res) => {
                console.log('Game rented');
                // This will update the game state to reflect the new rental status
                setGames(games.map(game => game.id === gameId ? { ...game, isRented: true, userId: currentUser.id } : game));
            })
            .catch((error) => {
    ...
    const returnHandler = (rentalId) => {
        ...
        axios.get(`http://localhost:8000/api/rental/users/${currentUser.id}`,  {gameId: rentalId} , { withCredentials: true })
            .then((res) => {
        ...
        axios.put(`http://localhost:8000/api/return/${currentRental.id}`, {}, { withCredentials: true })
            .then((res) => {
                console.log('Game returned');
                setGames(games.map(game => game.id === rentalId ? { ...game, isRented: false, userId: null } : game));
            })
            .catch((error) => {
                alert(`${error.response.data.message}`);
                        ...
                        <tr key={game.id}>
                            ...
                            <td>
                                {game.isRented && game.userId === currentUser.id ? (
                                    <button onClick={() => returnHandler(game.id)} className='btn btn-warning'>
                                        Return</button>
                                ) : (
                                    <button onClick={() => rentalHandler(game.id)} className='btn btn-primary' disabled={game.isRented}>Rent</button>
                                )}
                            </td>
                            <td>
                                <CreateReview passUserId={passUserId.id} gameId={game.id} gameTitle={game.title} />
                            </td>
                            <td><button className='btn btn-info'>Favorite</button></td>
                        </tr>
                    ))}
                </tbody>
            </table>
